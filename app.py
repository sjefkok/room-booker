...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    if not st.session_state.admin_authenticated:
        ...
        with col2:
            available_weeks = alloc.get_available_weeks()
            # Exclude weeks where deadline has passed — rooms already allocated
            blockable_weeks = [m for m in available_weeks if alloc.is_before_deadline(m)]
            week_options = {_week_label(m): m for m in blockable_weeks}
